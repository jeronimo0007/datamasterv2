# Synapse, Databricks e Azure ML — opcional (caro). Use enable_analytics_stack = true só quando precisar.
# Providers: az provider register -n Microsoft.Synapse --wait
...
locals {
  alnum_project = replace(var.project_name, "-", "")
}
...
resource "azurerm_synapse_workspace" "main" {
  ...
  name                                 = substr("${local.alnum_project}syn${random_id.analytics_suffix[0].hex}", 0, 50)
...
resource "azurerm_role_assignment" "synapse_storage_blob_contrib" {
  count = var.enable_analytics_stack ? 1 : 0
...
resource "azurerm_databricks_workspace" "main" {
  ...
  name                = substr("${local.alnum_project}dbw${random_id.analytics_suffix[0].hex}", 0, 30)
  resource_group_name = azurerm_resource_group.main.name
  location            = azurerm_resource_group.main.location
...
resource "azurerm_machine_learning_workspace" "main" {
  ...
  name                    = substr("${local.alnum_project}mlw${random_id.analytics_suffix[0].hex}", 0, 33)
  location                = azurerm_resource_group.main.location
  resource_group_name     = azurerm_resource_group.main.name
